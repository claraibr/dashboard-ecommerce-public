...
def plot_review(georeview_df):
  ...
  df_plot = georeview_df[['durasi_pengiriman', 'review_score']].dropna()
...
def main():
  ...
  st.header('1. Apakah terdapat Hubungan antara Jarak Lokasi Pengiriman dengan Tingkat Kepuasan Pelanggan?')
  st.subheader("Distribusi Tingkat Kepuasan Customer berdasarkan Durasi Pengiriman")
  st.pyplot(plot_review(georeview_df))
  st.subheader("Distribusi Tingkat Kepuasan Customer berdasarkan Kategori Jarak")
  st.pyplot(plot_review_jarak(georeview_df))
  st.subheader("Scatterplot Jarak vs Review Score")
  st.pyplot(scatter_review(georeview_df))
  st.header('2. Apakah terdapat pola khusus mengenai jumlah pembelian produk pada waktu tertentu?')
  st.subheader("Tren Jumlah Pesanan Harian")
  st.pyplot(plot_trend_harian(trend_perhari))
  st.subheader("Heatmap Jumlah Pemesanan per Bulan")
  st.pyplot(plot_trend_bulanan(trend_bulanan))
  st.subheader("Tren Jumlah Order berdasarkan Status per Bulan")
...
def main():
  ...
  if section == "Distribusi Kepuasan":
            ...
            rentang_waktu = st.date_input("Pilih Rentang Tanggal", [])
